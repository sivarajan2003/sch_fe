...
export default function GradePage() {
   const isLocked = false;// üîí enable full blur lock
    ...
    const csv =
      ...
      ["ID,Grade,Percentage,Points,Status"]
        .concat(
          data.map(
            (d) =>
              `${d.id},${d.grade ?? d.subject ?? "ó"},${d.percentage ?? d.exam_date ?? "ó"},${d.points ?? "ó"},${d.status}`
          )
        )
        .join("\n");
  ...
  >
        <div className="bg-white px-6 py-4 rounded-xl shadow-lg text-center">
          ...
          <p className="text-sm font-semibold text-gray-800">
            Subscription Upgrade Required ‚Äî Contact Atelier Creation
          </p>
  ...
  <div className="flex flex-col">
    ...
    <span className="text-sm font-medium">
      ...
      <div className="text-sm space-y-1">
        <p><span className="text-gray-500">Grade Points:</span> {d.points ?? "ó"}</p>
...
{openEdit && selectedGrade && (
  <AddGradeModal
    //initialData={selectedGrade}   // üëà pass existing data
    onClose={() => {
      setOpenEdit(false);
