...
export default function Verification() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 md:px-6">
              ...
              data.map((app) => (
                <tr key={app.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 text-blue-600 font-medium">
                    {String(app.displayId).substring(0, 20)}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs ${statusStyle(app.status)}`}>
                      {app.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 hidden md:table-cell flex items-center gap-2">
                    <FileText size={14} /> {app.documents}
                  </td>
                  <td className="px-6 py-4 text-center">
                    <button
                      onClick={() =>
                        navigate(`/admin/dashboard/receptionist/admissions/verification/${app.id}`)
                      }
                    >
      ...
      {/* PAGINATION */}
      <div className="flex justify-end items-center gap-3 px-6 py-4 border-t text-sm">
        <button
