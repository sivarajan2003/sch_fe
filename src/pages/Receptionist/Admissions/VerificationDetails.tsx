...
export default function VerificationDetails() {
  ...
  const markAllVerified = async () => {
    if (!app) return;
    try {
      // Update status to "Interview Scheduled" directly as "Documents Verified" is not a valid backend status
  ...
  return (
    <>
      ...
      {selectedDoc && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedDoc(null)}>
          <div className="bg-white rounded-xl w-full max-w-5xl h-[85vh] flex flex-col shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center px-4 py-3 border-b bg-gray-50">
              <h3 className="font-semibold text-gray-700">Document Viewer</h3>
              <div className="flex items-center gap-2">
                <a href={selectedDoc} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline mr-2">Open in New Tab</a>
                <button onClick={() => setSelectedDoc(null)} className="p-1.5 hover:bg-gray-200 rounded-full transition-colors">
                  <X size={20} className="text-gray-600" />
                </button>
              </div>
            </div>
            <div className="flex-1 bg-gray-100 p-1 flex items-center justify-center overflow-auto relative rounded-b-xl">
              {/* Simple heuristic: if it looks like an image, render img, else iframe */}
              {/\.(jpg|jpeg|png|gif|webp|svg)$/i.test(selectedDoc) ? (
                <img src={selectedDoc} className="max-w-full max-h-full object-contain shadow-lg" alt="Document Preview" />
              ) : (
                <iframe
                  src={selectedDoc}
...
function DocumentCard({ title, status, remarks, url, onVerify, onQuery, onView }: any) {
  return (
    <div className="border rounded-xl p-5 space-y-3">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          {url && (
            <button
              onClick={() => onView(url)}
              className="px-3 py-1 text-xs border border-blue-200 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors"
            >
              View Document
            </button>
          )}
      ...
      <textarea className="w-full border rounded-lg p-3 text-sm min-h-[60px] focus:ring-2 focus:ring-blue-500 outline-none transition-all" placeholder="Add remarks..." defaultValue={remarks} />
      <div className="flex flex-col sm:flex-row gap-3">
        <button onClick={onVerify} disabled={!url} className="flex-1 bg-green-600 text-white py-2 rounded-lg disabled:opacity-50 hover:bg-green-700 transition-colors">Verify</button>
        <button onClick={onQuery} disabled={!url} className="flex-1 bg-red-600 text-white py-2 rounded-lg disabled:opacity-50 hover:bg-red-700 transition-colors">Query</button>
      </div>
    </div>
  );
}
